...
from data.load_data import load_data, get_data_filepaths
...
#         m_metrics['digital_wage_rate_error'] = m['digital_wage_rate_error']
    return m_metrics
...
def evaluate_model(y_test,
    ...
    # Predict national digital wage rate
    digital_wage_rate_pred = None
    if (predict_dig_wage_rate == True) & (country is not None):
        ...
        digital_wage_rate_actual, digital_wage_rate_pred = predict_digital_wage_rate(TRAIN_PATH,
                                                TEST_PATH,
                                                model,
                                                selected_columns=selected_columns,
                                                show=False,
                                                return_values=True)
        metrics['digital_wage_rate_error'] = digital_wage_rate_pred - digital_wage_rate_actual
    else:
        metrics['digital_wage_rate_error'] = None
    ...
    if show is True:
        ...
        if (predict_dig_wage_rate == True) & (country is not None):
            print("Actual digital wage rate: {:0.2%} ".format(digital_wage_rate_actual))
            print("Predicted digital wage rate: {:0.2%} ".format(digital_wage_rate_pred))
    ...
    if (store_model is True) & (model_name is not None) & (country is not None):
        ...
        with open(filepath, 'wb') as f:
            if type(model) == KerasClassifier:
                ...
                model = model_path
            if 'digital_wage_rate_error' not in metrics.keys():
                metrics['digital_wage_rate_error'] = None
            output = {'model': model,
                      'y_true': y_test,
                      'y_pred': y_pred,
                      ...
                      'features': features,
                      'digital_wage_rate_error': metrics['digital_wage_rate_error'],
...
#         files = [f for f in files if f[-6:] != 'simple']
    ...
    results = display_model_comparison(metrics,
                                       show_roc=True,
                                       show_cm=False,
#                                        show_digital_wage_rate_error=False,
                                       transpose=True)


def predict_digital_wage_rate(train_path, test_path, model,
                         standardize_columns='numeric',
                         ravel=True,
                         selected_columns=None,
                         show=True,
                         return_values=False):
    # Recombine the entire dataset to get the actual digital wage rate
    ...
    X_test, y_test, w_test = load_data(test_path,
                                       ...
                                       ravel=ravel,
                                       selected_columns=selected_columns)
    digital_wage_rate = pd.DataFrame(np.vstack((np.vstack((y_train, w_train)).T,
                                       np.vstack((y_test, w_test)).T)),
                            columns=['receive_digital_wages', 'wtg'])
    digital_wage_rate_actual = (digital_wage_rate.wtg * digital_wage_rate.receive_digital_wages).sum() / digital_wage_rate.wtg.sum()

    # Make predictions on entire dataset to get the predicted digital wage rate
    digital_wage_rate['pred'] = model.predict(np.concatenate((X_train.to_numpy(), X_test.to_numpy())))
    digital_wage_rate_pred = (digital_wage_rate.wtg * digital_wage_rate.pred).sum() / digital_wage_rate.wtg.sum()

    if show == True:
        print("Actual digital wage rate: {:0.2%} ".format(digital_wage_rate_actual))
        print("Predicted digital wage rate: {:0.2%} ".format(digital_wage_rate_pred))
    if return_values:
        return digital_wage_rate_actual, digital_wage_rate_pred
    else:
        return
